...
void process( const Mat&orig, Mat&dst )
{
...
//    dst = cvCreateImage( cvSize(nWidth,nHeight), IPL_DEPTH_8U, 3 ); // 创建目标图像
    ...
    if ( orig.channels() == 3 )
    {
        for ( y = 0; y < nHeight; y++ )
            for ( x = 0; x < nWidth; x++ )
            {
                sImage[(y*nWidth+x)*orig.channels()] = orig.data[y*step+x*orig.channels()];
                sImage[(y*nWidth+x)*orig.channels()+1] = orig.data[y*step+x*orig.channels()+1];
                sImage[(y*nWidth+x)*orig.channels()+2] = orig.data[y*step+x*orig.channels()+2];
            }
    }
    memcpy( dImage, sImage, nWidth*nHeight*orig.channels());
    /* 彩色图像增强 */
    MSRCR( dImage, nWidth, nHeight, orig.channels() );
...
*/
    for ( y = 0; y < nHeight; y++ )
        for ( x = 0; x < nWidth; x++ )
        {
            dst.data[y*step+x*3] = dImage[(y*nWidth+x)*3];
            dst.data[y*step+x*3+1] = dImage[(y*nWidth+x)*3+1];
            dst.data[y*step+x*3+2] = dImage[(y*nWidth+x)*3+2];
        }
        /************** 显示处理图像 ****************/
