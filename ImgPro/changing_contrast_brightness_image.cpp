...
//
//  changing_brightness_contrast.cpp
//  LearnOpenCV
//
//  Created by Zhang,Qi(VIS) on 2020/9/22.
//  Copyright © 2020 Zhang,Qi(VIS). All rights reserved.
//
...
namespace {
    int alpha = 100;
    int beta = 100;
    double gamma_cor = 100;

    Mat img_original,img_corrected,img_gamma_corrected;
    
    //线性亮度和对比度调整，beta会使像素之间的差值变大，即对比度增强，alpha则使像素亮度值变大
    ...
    //  非线性的gamma矫正，每个像素所调整的范围都是不一样的，暗的像素调整的更多，亮的像素调整的更少
    void gammaCorrection(const Mat &img,const double gamma_)
    {
        ...
        for( int i = 0;i < 256;++i)
        {
            p[i] = saturate_cast<uchar>(pow(i / 255.0,gamma_)*255.);
        }
        Mat res = img.clone();
